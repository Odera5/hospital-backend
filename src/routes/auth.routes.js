...
const normalizeProcedurePresetPrices = (value) => {
  ...
  return incoming.map((preset, index) => {
    const fallback =
      DEFAULT_PROCEDURE_PRESETS[index] || DEFAULT_PROCEDURE_PRESETS[0];
    const unitPrice = Number(preset?.unitPrice);

    return {
      description: String(
        preset?.description || fallback.description || "",
      ).trim(),
      category:
        String(preset?.category || fallback.category || "service").trim() ||
        "service",
      unitPrice:
...
const serializeClinic = (clinic) =>
  clinic
    ? {
        ...
        procedurePresetPrices: normalizeProcedurePresetPrices(
          clinic.procedurePresetPrices,
        ),
        ...
        reminderOffsets: Array.isArray(clinic.reminderOffsets)
          ? clinic.reminderOffsets
          : [1440, 120],
        ...
        branchCount: Array.isArray(clinic.branches)
          ? clinic.branches.length
          : clinic._count?.branches || 0,
...
const resolveValidAssignedBranchIds = async ({
  ...
  const normalizedAssignedBranchIds =
    normalizeAssignedBranchIds(assignedBranchIds);
  ...
  if (invalidBranchId) {
    return {
      error: {
        status: 400,
        message:
          "Assigned branches must belong to this clinic and remain active.",
      },
      branchIds: [],
    };
  }
...
const doAssignedBranchesFitWithinActorScope = (
  actorAssignedBranchIds,
  targetAssignedBranchIds,
) => {
  const actorBranchSet = new Set(
    normalizeAssignedBranchIds(actorAssignedBranchIds),
  );
  const targetBranchIds = normalizeAssignedBranchIds(targetAssignedBranchIds);

  return (
    targetBranchIds.length > 0 &&
    targetBranchIds.every((branchId) => actorBranchSet.has(branchId))
  );
};
...
const createIntakePublicToken = () => crypto.randomBytes(24).toString("hex");
  ...
  try {
    ...
    res.json({
      clinic: {
        ...
        intakeEnabled: isEnterprise
          ? Boolean(currentBranch?.intakeEnabled)
          : Boolean(clinic.intakeEnabled),
        intakePublicToken: isEnterprise
          ? currentBranch?.intakePublicToken || null
          : clinic.intakePublicToken || null,
      },
      activeBranch: currentBranch
        ? serializeBranch(currentBranch)
  ...
  async (req, res) => {
    try {
      ...
      if (typeof requestedEnabled !== "boolean") {
        return res
          .status(400)
          .json({ message: "intakeEnabled must be provided as true or false" });
      }
      ...
      if (isEnterprise) {
        ...
        if (!currentBranch) {
          return res
            .status(404)
            .json({
              message: "No active branch is available for this staff account.",
            });
        }
      ...
      res
        .status(500)
        .json({ message: "Failed to update patient intake link settings" });
    }
  },
);

router.post(
  "/clinic-profile/intake-link/regenerate",
  protect,
  ...
  async (req, res) => {
    try {
      ...
      if (isEnterprise) {
        ...
        return res.json({
          message:
            "Patient intake link regenerated successfully for this branch",
          clinic: {
            intakeEnabled: branch.intakeEnabled,
            intakePublicToken: branch.intakePublicToken,
          },
      ...
      res
        .status(500)
        .json({ message: "Failed to regenerate patient intake link" });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (!password) {
        return res
          .status(400)
          .json({ message: "Password is required to initiate deactivation" });
      }
      ...
      res
        .status(500)
        .json({ message: "Failed to initiate clinic deactivation" });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        console.error(
          "Clinic registration verification email error:",
          emailError,
        );
        emailWarning =
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.json(
        filterManageableStaffForActor(req.user, staff).map(serializeUser),
      );
    } catch (error) {
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to update staff branch assignments" });
    }
  },
);
...
router.post("/refresh-token", async (req, res) => {
  ...
  try {
    ...
    if (!hasActiveProAccess(user.clinic) && user.role === "admin") {
      const sessionId = user.refreshToken.substring(
        user.refreshToken.length - 15,
      );
